...
char* get_db_path() {
    ...
    if (!full_path) {
        free(home);
        fprintf(stderr, "Error: could not allocated memory for the db path buffer.\n");
        return NULL;
    }
...
int save_db(json_object* db) {
    ...
    if (!json_as_string) {
        fprintf(stderr, "Error: failed to convert the JSON object (db) to string.\n");
        fclose(fp);
        free(full_path);
        return -1;
    }
...
int create_db() {
    ...
    if (!db) {
        fprintf(stderr, "Error: failed to allocate JSON object (db).\n");
        free(full_path);
        return -1;
    }
...
int tag_exists(const char* tag) {
    ...
    if (!all_tags_entry || !json_object_is_type(all_tags_entry, json_type_array)) {
        fprintf(stderr, "Warning: the entry for \"%s\" does not exist yet or seems to be corrupted.\n", ALL_TAGS_KEY);
        json_object_put(db);
        return 0;
    } 

    const int len = json_object_array_length(all_tags_entry);  
    for (int i = 0; i < len; i++) {
        json_object* current_tag = json_object_array_get_idx(all_tags_entry, i);
